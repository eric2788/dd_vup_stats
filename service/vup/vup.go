...
func MigrateToVup(uid int64) {
	...
	var watcherBehaviours []db.WatcherBehaviour
	...
	if err != nil {
		logger.Errorf("從 watcher_behaviour 表中獲取 UID:%v 的行為記錄時出現錯誤: %v", uid, err)
		return
	}
	...
	behaviours := make([]*db.Behaviour, len(watcherBehaviours))

	for i, b := range watcherBehaviours {
		behaviours[i] = b.ToBehaviour()
	}

	logger.Debugf("即將遷移 %d 筆記錄到 behaviour 表", len(behaviours))
	...
	if result.Error != nil {
		logger.Errorf("將 UID:%v 的行為記錄從 watcher_behaviour 表中移動到 behaviour 表時出現錯誤: %v", uid, err)
		return
	} else if result.RowsAffected > 0 {
		logger.Infof("成功將 UID:%v 的 %d 筆行為記錄從 watcher_behaviour 表中移動到 behaviour 表", uid, result.RowsAffected)
	}

	logger.Debugf("即將刪除 UID:%v 的 %d 筆記錄", uid, len(watcherBehaviours))

	// delete records from watcher_behaviour table
	result = db.Database.
		Where("uid = ?", uid).
		Delete(&db.WatcherBehaviour{})

	if result.Error != nil {
		logger.Errorf("刪除 UID:%v 的行為記錄時出現錯誤: %v", uid, err)
	} else if result.RowsAffected > 0 {
		logger.Infof("成功從 watcher_behaviour 刪除 UID:%v 的 %d 筆行為記錄", uid, result.RowsAffected)
	}
